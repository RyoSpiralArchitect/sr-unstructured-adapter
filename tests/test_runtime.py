...
def test_runtime_normalize_and_stats() -> None:
    ...
    blocks = [
        Block(type="paragraph", text="Hello\n\nWorld", source="test", confidence=0.4),
        Block(type="heading", text="TITLE", source="test", confidence=0.9),
    ]
...
def test_runtime_layout_analysis_records_stats() -> None:
    ...
    if not runtime.layout_enabled:
        return
    block = Block(type="paragraph", text="body", source="test", confidence=0.5)
