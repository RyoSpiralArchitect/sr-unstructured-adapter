# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [0.2.4] - 2026-02-26

### Added
- Optional embedding-based related-context retrieval for LLM escalation prompts (configure via `llm.context.top_k`).

## [0.2.3] - 2026-02-26

### Added
- Context-aware embeddings (`BlockEmbedder.embed_with_context`) that inject deterministic semantic-field statistics.

### Changed
- The fallback text embedding path now uses token-hash vectors (no heavy ML deps) instead of random projections.

## [0.2.2] - 2026-02-26

### Added
- Explicit structural confidence (`attrs.confidence_structural`) and semantic alias (`attrs.confidence_semantic`) signals wired into the escalation gate and logs.

### Changed
- Escalation meta-model defaults now treat structural confidence as the primary feature (`confidence_structural`) and bump feature version to `v2`.
- Benchmark harness expanded to ablate structural vs semantic gates.

## [0.2.1] - 2026-02-26

### Added
- Streaming upload endpoint (`POST /convert-stream`) that emits NDJSON events (`document`, `block`, `summary`) without LLM escalation.

## [0.2.0] - 2026-02-26

### Added
- Optional FastAPI API server (`sr-adapt-api`) with request ID, upload-size cap, rate limiting, and API key support.
- Jobs subsystem (in-memory + SQLite backend) for async-style API operation.
- Deterministic semantic confidence scoring, wired into the escalation policy as an additional gate.

### Changed
- CI defaults to running without the native runtime (environment parity with Linux runners).
